const game = {
    ...
    // --- PHASE 7: REVEAL (MYTHOLOGICAL ARCHETYPES) ---
    calcArchetype() {
        let name = "POTTAN (THE OUTCAST)";
        let quote = "Truth is a fire that burns the holder.";
        let desc = "Defiant truth-teller from marginalized roots. You rejected the system's binary choices and exposed its flaws. You invite self-doubt but stand resilient against rejection.";
        let maskFile = "potton.png"; // Default / Chaos

        const s = this.scores;

        // LOGIC FOR MASKS
        // 1. MUCHILOT (Healer) -> High Empathy
        if (s.empathy >= 3 && s.ruthless < 3) {
            name = "MUCHILOT BHAGAVATI (THE HEALER)";
            quote = "To heal is to absorb the pain of others.";
            desc = "Nurturing leader embodying collective trauma. You boosted empathy and chose to save the many. Your burden is heavy, but your spirit remains a sanctuary.";
            maskFile = "muchilot.png";
        }
        // 2. CHAMUNDI (Avenger) -> High Ruthless
        else if (s.ruthless >= 4) {
            name = "CHAMUNDI (THE AVENGER)";
            quote = "Rage is the fuel of justice.";
            desc = "A fierce maternal figure wrestling suppressed rage. You shielded your interests but risked sanity in vengeful outbursts. Forgiveness is not in your nature.";
            maskFile = "chamundi.png";
        }
        // 3. GULIKAN (Vigilant) -> Balanced / High Survival
            ...
            desc = "Eternal guardian haunted by isolation. You excelled in foresight, carefully weighing every wager. Your paranoia kept you safe, but at the cost of connection.";
            maskFile = "gulikan.png";
        }
        // Default fallthrough is POTTAN (Chaos/Outcast)

        // --- EXPANDED REVEAL INFO ---
        let fate = "UNKNOWN";
        if (maskFile === "potton.png") fate = "You will burn the system down, but you will not survive the fire.";
        if (maskFile === "muchilot.png") fate = "You will save many, but you will lose yourself in their grief.";
        if (maskFile === "chamundi.png") fate = "You will conquer all, only to realize you are ruling a graveyard.";
        if (maskFile === "gulikan.png") fate = "You will survive, but you will be the only one left.";
        ...
        // Flicker effect for reveal
        setTimeout(() => maskImg.style.opacity = 1, 500);

        document.getElementById('arch-stats').innerHTML = `
            <p>EMPATHY: ${s.empathy}</p>
            <p>RUTHLESSNESS: ${s.ruthless}</p>
            <p>CHAOS: ${s.chaos}</p>
            <p>TRUST: ${s.trust}</p>
            <div style="margin-top: 20px; border-top: 1px solid var(--accent-red); padding-top: 10px;">
                <p style="color: var(--accent-rust); font-weight: bold;">SECRET FATE:</p>
                <p style="font-style: italic; font-size: 0.9rem;">"${fate}"</p>
            </div>
        `;
    },
    ...
    activatePhase(newPhase) {
        ...
        const bgMap = {
            ...
            'prelude': 'assets/bgVideo.png', // New Prelude BG
            'cards': 'assets/bgTrolley.png',
        ...
        if (this.infoTexts && this.infoTexts[newPhase]) { // Added null check for infoTexts
